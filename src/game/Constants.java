package game;

public class Constants {
    static final int BLOCK_SIZE = 64;
    static final int CHARACTER_SIZE = 48;
    static final int ENEMY_SIZE = 32;


    public static final String ICON_IMAGE = "../images/icon.ico";

    public static final String CHARACTER_IMAGE = "../images/bmenStend.png";
    static final String DEAD_CHARACTER_IMAGE = "./src/images/brokenMen.png";
    static final String ENEMY_IMAGE = "./src/images/enemyStay.png";
    static final String BOMB_IMAGE = "./src/images/bomb1.png";
    public static final String UNBREAKABLE_BLOCK_IMAGE = "../../images/unB.png";
    public static final String BREAKABLE_BOCK_IMAGE = "../../images/br.png";
    public static final String BACKGROUND_BLOCK_IMAGE = "../../images/bg.png";

    public static final String LEVEL_1_PATH = "levels/level1";
}

//todo зависить от объектов - не по-пацански
// работаем с интерФЭЙСАМИ пасоны
//

//todo